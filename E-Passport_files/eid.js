...
function openPaymentWindow()
{
	if (bowser.msie) 
	{
		 window.open("paymenttermsandconditions.xhtml",'E-Passport Terms and Conditions','height=820,width=830,location=no,toolbar=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,left=500,top=80').focus();
	}
	else 
	{
	  var ret = window.showModalDialog("paymenttermsandconditions.xhtml", "E-Passport Terms and Conditions", "dialogWidth:830px;dialogHeight:820px");
	}
...
function printDivInNewWindow(divName) {
	...
	var receiptHeader = "<table class='table'><tr>" +
			...
			"<div ><img src='./images/PAK-ID-LOGO.png' width='250'/></div></td></tr>" +
